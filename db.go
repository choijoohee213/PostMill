...
// SetAutoGenerated는 AI가 상품까지 고른 초안을 저장한다. 답글은 하나라
// 두 번째 답글 칸은 비운다.
...
func (db *DB) SetAutoGenerated(ctx context.Context, id int64, d AutoDraft) error {
	_, err := db.pool.Exec(ctx,
		`UPDATE posts SET product_name = $2, product_url = $3, body = $4, detail = $5,
		 detail2 = '', status = $6, error_msg = '',
		 affiliate_link = CASE WHEN $7 <> '' THEN $7 ELSE affiliate_link END,
		 link_auto = (link_auto OR $7 <> ''), taca_item_id = $8
		 WHERE id = $1`,
		id, d.ProductName, d.ProductURL, d.Body, d.Detail, StatusPending, d.AffiliateLink, d.TacaItemID)
	return err
}
