...
var hookTypes = []hookType{
	...
	// 단점을 먼저 솔직하게 말하는 글이 광고 같지 않아 전환이 잘 난다.
	{"솔직형", `첫 줄은 사기 전에 망설였던 점이나 아쉬운 점부터 솔직하게 꺼낸다. 둘째 줄에 그런데도 계속 쓰는 이유 하나, 마지막 줄은 읽는 사람에게 묻는다.`},
}
...
// polish는 모델 출력을 저장할 모양으로 다듬는다.
func polish(s string) string { return varyLaughs(tighten(s)) }

// laughRun은 두 글자 이상 이어진 ㅋ, ㅠ다.
var laughRun = regexp.MustCompile(`ㅋ{2,}|ㅠ{2,}`)

// 사람은 ㅋㅋ, ㅠㅠ를 기분 따라 길게도 짧게도 쓴다. 짧은 쪽이 더 흔하다.
var (
	laughLens = []int{2, 3, 3, 4, 5, 6}
	sobLens   = []int{2, 2, 3, 4, 5}
)

// varyLaughs는 ㅋㅋ, ㅠㅠ 길이를 섞는다. 프롬프트로 일러도 모델은 늘 두 글자로
// 써서 AI가 쓴 티가 난다.
func varyLaughs(s string) string {
	return laughRun.ReplaceAllStringFunc(s, func(run string) string {
		lens := laughLens
		if strings.HasPrefix(run, "ㅠ") {
			lens = sobLens
		}
		r, _ := utf8.DecodeRuneInString(run)
		return strings.Repeat(string(r), lens[rand.IntN(len(lens))])
	})
}
...
- 제품의 기능, 구조, 소재의 성질, 크기, 무게, 튼튼함, 소리, 냄새, 세척이나 관리,
  구성품, 성능(보온·보냉이 얼마나 가는지, 냄새를 얼마나 잡는지, 뜨거운 게 안 느껴지는지 같은
  효과)은 확인된 정보에 적힌 것만 쓴다.
...
- 확인된 정보에 없으면 이런 문장은 쓰지 않는다:
  ...
  "물때가 안 낌", "세척이 쉬움", "생각보다 가벼움", "얼음이 하루 종일 안 녹음",
  "냄새를 싹 잡아줌", "손이 하나도 안 뜨거움"
...
// pickRules는 AI가 상품을 고를 때의 기준이다. 싸고 자주 사고 실패해도
// 아깝지 않은 물건일수록 링크를 누르는 문턱이 낮다.
const pickRules = `- 2만원에서 5만원 사이의 물건이 좋다.
...
const reviewFlow = `    답글은 후기처럼 이 순서로 흐른다. 각각 한 줄이면 충분하다:
    ...
    2) 아쉬운 점 하나를 솔직하게. 확인된 정보에 없는 결함을 지어내지 말고
       망설임이나 내 사정으로 말한다. "솔직히 이 돈 주고 살 건가 싶었어",
       "처음엔 좀 어색하더라", "없어도 사는 데 지장은 없긴 해" 같은 식.
    3) 그래도 어떤 사람한테 맞는지. "자취하는 사람", "아침마다 정신없는 사람"처럼 구체적으로.
       "~한테 딱임"으로 매번 끝내지 말고 말을 바꾼다.
    4) 마지막 줄은 부담 없는 안내 한마디. 링크가 바로 아래 답글로 달린다.
       "자세한 건 밑에", "옵션은 아래서 봐", "궁금하면 밑에 달아둘게"처럼 정보만 알려주고
       "꼭 사", "지금 아니면 늦음" 같은 재촉은 하지 마라. 매번 같은 말을 쓰지 마라.
`

// humanTone은 AI가 쓴 티를 없애는 규칙이다. 모델은 ㅋㅋ, ㅠㅠ를 늘 두 글자로
// 쓰고 줄마다 같은 어미로 끝내서 한눈에 티가 난다.
const humanTone = `AI가 쓴 티가 나지 않게 (자주 틀리는 부분이다):
- ㅋ, ㅠ 길이는 그때그때 다르게. ㅋㅋ만 쓰지 말고 ㅋㅋㅋ, ㅋㅋㅋㅋㅋ, ㅠㅠㅠㅠ처럼
  기분 따라 짧게도 길게도 쓴다. 한 부분에 한두 번이면 충분하고 안 써도 된다.
  ㅋ 한 글자는 비꼬는 것처럼 차갑게 읽히니 쓰지 마라.
  만족스러운 얘기엔 ㅋㅋ ㅎㅎ ! ~, 불편했던 얘기엔 ㅠㅠ ;; 를 쓴다.
- 20대가 친구한테 카톡하듯 쓴다. 어미는 "~했어", "~더라", "~거든", "~잖아",
  "~인 듯", "~는데"처럼 말하는 반말을 주로 섞고 말을 끝까지 안 맺어도 된다.
- 음슴체("~함", "~임", "~음")는 한 글에 한 번 넘게 쓰지 마라. 줄마다 음슴체로 끝나면
  옛날 커뮤니티 글처럼 보인다.
- 요즘 말투를 한 부분에 한 번 정도 자연스럽게 섞는다. "ㄹㅇ", "진심", "개편함"처럼
  "개-"를 붙이는 말, "미쳤다", "나만 몰랐나", "~각", "살짝" 같은 것. 매 줄에 넣으면
  오히려 억지스러우니 많아야 한두 개.
- 줄 길이를 들쭉날쭉하게. 한두 단어짜리 줄도 괜찮다.
- 교훈이나 요약으로 깔끔하게 맺지 마라. "결론은", "한마디로", "삶의 질" 같은 말 금지.
