...
사용자별로 저장하고 만료 전에 자동으로 갱신한다.

OAuth 승인 흐름을 쓰지 않는 이유: Meta는 승인 화면에서 계정을 고르는 방법을
제공하지 않는다. 인스타 계정이 여러 개 묶여 있으면 브라우저에 남은 세션으로
즉시 처리되고, 그 계정이 테스터가 아니면 다시 로그인할 기회 없이 오류로 끝난다.
`force_reauth`도 무시된다. 앱 검수를 통과하면 이 제약이 없어진다.

토큰은 Meta 앱에 `Threads 테스터`로 초대해 수락한 계정에 대해서만 발급된다.
...
DB를 쓰는 통합 테스트는 `TEST_DATABASE_URL`이 설정되어 있을 때만 실행되고,
없으면 건너뛴다. `DATABASE_URL`로 폴백하지 않는다.

이 테스트들은 `app_state`의 스레드 토큰을 덮어쓰고 `posts`에 행을 만들기
때문에, 운영 DB에 연결되면 실제 스레드 연결이 끊기고 발행이 실패한다.
실제로 그런 일이 있었다.

Neon에서 브랜치를 하나 만들어 그 주소를 `TEST_DATABASE_URL`에 넣으면 된다.
